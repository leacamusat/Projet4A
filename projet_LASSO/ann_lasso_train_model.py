...
def lambda_qut_sann_classification2(
	  xsample,hat_p_training,nSample=100,
      miniBatchSize=100,alpha=0.05,option='quantile'):
 Returns the lambda of LASSO penalty that with a probability 1-alpha set W_1 to 0.

            Parameters:
                    x_sample : A decimal integer
		    hat_p_training (n_classes list) : a vector of probability corresponding to the probability to belong to each class according to the training dataset
		    n_sample (int)
                    miniBatchSize :
		    alpha : 
		    option : 
    ...
    #hat_p_training is a vector of probabilitity of the percentage of each class
    number_class=len(hat_p_training)
    print(number_class)
    fullList = np.zeros((miniBatchSize*(nSample//miniBatchSize+offset),))
    print(np.shape(fullList), )
    for index in range(nSample//miniBatchSize+offset):
        
        ySample = np.random.multinomial(1,hat_p_training,size=(n,1,miniBatchSize)) #prédit des array de classes de manière aléatoire (pour chaque point du tableau une classe est prédite)
        y_mean = np.mean(ySample,axis=0) 
        print("ySample", ySample, np.shape(ySample), "y_mean", y_mean.shape)
        print('miniBatch',miniBatchSize)
        xy = np.zeros(shape=(n,p1,miniBatchSize,number_class))
        for index_n in np.arange(n):
            print('xy reduit', xy[index_n,:,:,:].shape)
            print(xsample[index_n,:].shape, (y_mean-ySample)[index_n,:,:].shape, np.outer(xsample[index_n,:],
              (y_mean-ySample)[index_n,:,:]).shape)
            xy[index_n,:,:,:]=np.outer(xsample[index_n,:],
              (y_mean-ySample)[index_n,:,:]).reshape((p1,
              miniBatchSize,number_class))
                                                      
        print(tf.reduce_sum(xy,axis=0).numpy().shape)
        print(tf.reduce_sum(np.abs(tf.reduce_sum(xy,axis=0).numpy()),
          axis=2).shape)
        
        print(np.amax(tf.reduce_sum(np.abs(tf.reduce_sum(xy,axis=0).numpy()),
          axis=2).numpy(),axis=0)
.shape)
        ...
        x = tf.constant(0.)
        with tf.GradientTape() as g: #automatic differentiation 
        	g.watch(x)
        	y=activation_function(x)
        dy_dx=g.gradient(y,x) #on différencie y en fonction par rapport à x et on le prend en 0
        print(dy_dx )
        stat=dy_dx * xymax
        print(np.shape(stat))
        print(index*miniBatchSize-(index+1)*miniBatchSize)
...
def lambda_qut_sann_classification_loop(
	  xsample,hat_p_training,nSample=100,
      miniBatchSize=100,alpha=0.05,option='quantile'):
    if np.mod(nSample,miniBatchSize)==0:
    	offset=0
    ...
    number_class=len(hat_p_training)
    print(number_class)
    fullList = np.zeros(miniBatchSize)
    print(np.shape(fullList), )
    for index in range(nSample//miniBatchSize+offset):
...
.shape)
        ...
        print(index*miniBatchSize-(index+1)*miniBatchSize)
        fullList+= stat
    fullList=fullList/range(nSample//miniBatchSize+offset)
...
# def lambda_qut_sann_classification_loop1(
# 	  xsample,hat_p_training,nSample=100,
#       miniBatchSize=100,alpha=0.05,option='quantile'):
#     if np.mod(nSample,miniBatchSize)==0:
#     	offset=0
...
def lambda_qut_sann_classification(
	  xsample,hat_p_training,nSample=1000,
      miniBatchSize=500,alpha=0.05,option='quantile'):
	''' Returns the lambda of LASSO penalty that set W_1 to 0 with a probability of  1-alpha
            Parameters:
                    x_sample : corresponds to the matrix x of inputs 
                    hat_p_training (n_classes list) : a vector of probability corresponding to the probability to belong to each class according to the training dataset
                    n_sample (int) : Monte Carlo simulations number 
                    miniBatchSize : 
                    alpha : the risk of the test hypothesis 
                    option :   	
                        -"quantile" : return the 1-alpha quantile of the computed list of lambdas
			-"full" : return the list of the computed lambdas 
	...
	#generalized value of y : we generate a multinomial distribution  
        y_mean = np.mean(ySample,axis=0)
        ...
        with tf.GradientTape() as g:
            g.watch(x)
            y=activation_function(x)
        dy_dx=g.gradient(y,x)#gradient of the function of activation automatically computed 
...
def TFmodel(Xsample,Ysample,p2,activation_function,lamb,learningRate,
      withSD,ifTrain=True,iterationISTA=3000,  iniscale=0.01, 
    ...
    return [w1,w2,c, p_hat, y_hat,cost,learningRate, p_hat]
...
def TFmodelfit(X,y,p2,activation_function,lamb,learningRate0,withSD,ifTrain=True,iterationISTA=3000, Xtest=None, iniscale=0.01):
    ...
    lro=learningRate0[0]
    [w10,w20,c0,p_hat_0,y_hat_0,cost_0,lro_0, p_hat_0] = TFmodel(X,y,p2,activation_function,lamb,lro,withSD=False,ifTrain=True,iterationISTA=-999, iniscale=iniscale)
    [w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro_o, p_hato_o] = TFmodel(X,y,p2,activation_function,lamb,lro_0,withSD,ifTrain=True,iterationISTA=iterationISTA, iniscale=iniscale,w1_ini=w10, w2_ini=w20, c_ini=c0)
    
    for lr in learningRate0[1:]:
        [w1oo,w2oo,coo,p_hat_oo,y_hat_oo,cost_oo,lro_oo, p_hatoo]= TFmodel(X,y,p2,activation_function,lamb,lr,withSD,ifTrain=True,iterationISTA=iterationISTA, iniscale=iniscale,w1_ini=w10, w2_ini=w20, c_ini=c0)
        if (cost_oo<cost_o):
            cost_o=cost_oo
            w1o=w1oo
            w2o=w2oo
            ...
            break
    return [w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro_o, p_hat_o] 

def resultanalysis(w1o, needles_index_true=None):
    p1=w1o.shape[1]
    
    needles_index_hat=np.array(np.where(np.sum(abs(w1o[:,0:(p1-1)]),axis=0)>0),dtype=float)+1
    neuron_index=np.array(np.where(np.sum(abs(w1o[:,0:(p1-1)]),axis=1)>0),dtype=float)+1
    ...
    return needles_index_hat, neuron_index


def computeResults(x_train,y_train,learningRate_list,iniscale,lambda_qut,p2):
    ipath=-1
    lambi=np.exp(ipath)/(1+np.exp(ipath))*lambda_qut
    [w1o,w2o,co,p_hat_o,y_hat_o,p_test_hat_o,y_test_hat_o,cost_o,lro, p_hat_o]=TFmodelfit(x_train,y_train,p2,activation_function,lambi,learningRate_list,True,ifTrain=True,iterationISTA=3000, Xtest=None, iniscale=iniscale)
    for ipath in np.arange(5):
        lambi=np.exp(ipath)/(1+np.exp(ipath))*lambda_qut
        [w1o,w2o,co,p_hat_o,y_hat_o,p_test_hat_o,y_test_hat_o,cost_o,lro, p_hat_o]=TFmodel(x_train,y_train,p2,activation_function,lambi,lro,
          True,ifTrain=True,iterationISTA=3000, Xtest=None,
          rel_err=1.e-9, w1_ini=w1o, w2_ini=w2o, c_ini=co)
    if np.any(x_test)==None:
        [w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro, p_hat_o]=TFmodel(x_train,y_train,p2,activation_function,lambda_qut,lro,
          False,ifTrain=True,iterationISTA=10000,  
          rel_err=1.e-12, w1_ini=w1o, w2_ini=w2o, c_ini=co)
    else:
        [w1o,w2o,co,p_hat_o,y_hat_o,p_test_hat_o,y_test_hat_o,cost_o,lro, p_hat_o]=TFmodel(x_train,y_train,p2,activation_function,lambda_qut,lro,
          False,ifTrain=True,iterationISTA=10000, iniscale=0.0, 
          rel_err=1.e-12, w1_ini=w1o, w2_ini=w2o, c_ini=co)
    y_test=y_test.to_numpy()
    needles_index_hat,tpr,fdr,hat_equal_true,confusion_matrix,accuracy, neuron_index=resultanalysis(w1o)
    return w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro,needles_index_hat,tpr,fdr,hat_equal_true,confusion_matrix,accuracy, neuron_index, p_hat_o

def test_model_learnt(Xtest, w1, w2_l2normalized, 
if np.any(Xtest):
