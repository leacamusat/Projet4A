...
def lambda_qut_sann_classification(xsample,hat_p_training,nSample=1000,  miniBatchSize=500,alpha=0.05,option='quantile'):
...
def TFmodelfit(X,y,p2,activation_function,lamb,learningRate0,withSD,ifTrain=True,iterationISTA=3000, iniscale=0.01):
...
def computeResults(x_train,y_train,learningRate_list,iniscale,lambda_qut,p2):
    ...
    [w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro, p_hat_o]=TFmodelfit(x_train,y_train,p2,activation_function,lambi,learningRate_list,True,ifTrain=True,iterationISTA=3000, iniscale=iniscale)
    for ipath in np.arange(5):
        lambi=np.exp(ipath)/(1+np.exp(ipath))*lambda_qut
        [w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro, p_hat_o]=TFmodel(x_train,y_train,p2,activation_function,lambi,lro,
          True,ifTrain=True,iterationISTA=3000,
    ...
    needles_index_hat, neuron_index=resultanalysis(w1o)
    
    return w1o,w2o,co,p_hat_o,y_hat_o,cost_o,lro,needles_index_hat, neuron_index, p_hat_o

def test_model_learnt(Xtest,  w1, w2, number_class, c):
        w2_l2normalized=tf.nn.l2_normalize(tf.cast(w2,tf.float64),1)
        ...
        return y_test_hat, p_test_hat, mu_2layer_test
